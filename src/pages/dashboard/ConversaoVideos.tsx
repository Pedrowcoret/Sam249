    ...
    try {
      ...
      const response = await fetch('/api/conversion/convert', {
        ...
        body: JSON.stringify({
          ...
          overwrite: conversionSettings.overwrite
        })
      });
      ...
      if (result.requires_confirmation && !conversionSettings.overwrite) {
        if (confirm('Já existe uma conversão com essas configurações. Deseja sobrescrever o arquivo existente?')) {
          setConversionSettings(prev => ({ ...prev, overwrite: true }));
          // Tentar novamente com overwrite ativado
          startConversion();
        }
        return;
      }
